...
class LIFLayer(nn.Module):
    def __init__(self, tau=3.0, threshold=0.25, beta=15.0, dt=1.0):
        super().__init__()
...
class SNNEventModel(nn.Module):
    ...
    def forward(self, x):
        I = self.gain * self.bank(self.x_scale * x)   # gain always applied; in eval just frozen
        z, s = self.lif(I)
        return {"I": I, "z": z, "s": s, "logits": z.amax(dim=1)}  # [B,L]

    @torch.no_grad()
    def p_update_epoch(self, spike_sum, target_sum, spike_sites):
        err = target_sum / spike_sites - spike_sum / spike_sites   # want sum(s) ~= sum(y)
...
# loss
# =========================
def event_loss(logits, y, lambda_neg=0.1):
    ...
    return pos_term + lambda_neg * neg_term

# =========================
# train / eval
# =========================
@torch.no_grad()
def evaluate(model, loader, device, lambda_neg=0.1):
    model.eval()
    total, n, spk, tgt = 0.0, 0, 0.0, 0.0
    for xb, yb in loader:
        xb, yb = xb.to(device), yb.to(device)
        out = model(xb)
        total += event_loss(out["logits"], yb, lambda_neg=lambda_neg).item() * xb.size(0)
        n += xb.size(0)
        spk += out["s"].sum().item()
        tgt += yb.sum().item()
    return total / n, spk, tgt

def fit(model, train_loader, test_loader, epochs=20, lr=1e-3, device="cpu", lambda_neg=0.1):
    ...
    for ep in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            xb, yb = xb.to(device), yb.to(device)
            out = model(xb)
            loss = event_loss(out["logits"], yb, lambda_neg=lambda_neg)
        ...
        err = model.p_update_epoch(train_spk, train_tgt, train_sites)   # only once per epoch

        tr_loss = total / n
        te_loss, te_spk, te_tgt = evaluate(model, test_loader, device, lambda_neg=lambda_neg)
...
model, hist = fit(model, train_loader, test_loader, epochs=100, lr=1e-3, device=device, lambda_neg=0.1)
