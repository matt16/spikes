...
class SNNEventModel(nn.Module):
    ...
    def forward(self, x):
        ...
        z, s = self.lif(I)
        return {"I": I, "z": z, "s": s, "logits": z.amax(dim=1)}  # logits: [B,L]
...
def event_loss(logits, z, y, lambda_neg=0.1, lambda_cdf=0.5, lambda_dup=0.05):
    ...
    # cumulative alignment: penalizes early over-firing / late misses
    p = torch.sigmoid(logits)
    L_cdf = (p.cumsum(dim=1) - y.cumsum(dim=1)).abs().mean()

    # duplicate penalty: more than ~1 soft spike across filters at same time is discouraged
