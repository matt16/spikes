...
class SNNEventModel(nn.Module):
    ...
    def forward(self, x):                                # x: [B,2,L]
        Ia = self.gain * self.bank_a(self.x_scale * x[:, 0:1])
        Ib = self.gain * self.bank_b(self.x_scale * x[:, 1:2])
...
def event_loss(z, s, y, move_scale=2.0, miss_cost=1.0, fp_cost=1.0, lambda_birth=0.4, birth_r=1, lambda_dup=0.05):
    B, C, L = z.shape
    idx = torch.arange(L, device=z.device, dtype=z.dtype)
    norm = max(L - 1, 1)

    e_hard = (s.sum(dim=1) > 0).to(z.dtype)                 # [B,L] echte Event-Spikes
    e_soft = 1.0 - (1.0 - torch.sigmoid(z)).prod(dim=1)     # [B,L] surrogate Event-Prob
    e = e_hard + e_soft - e_soft.detach()                   # straight-through

    loss = z.new_tensor(0.0)
    birth = z.new_tensor(0.0)
    n_main, n_birth = 0, 0
    ...
    for b in range(B):
        ...
        # latency nur für gematchte echte spikes
        for ti, pj in pairs:
            lo, hi = max(0, pj - birth_r), min(L, pj + birth_r + 1)
            w = e_soft[b, lo:hi]
            w = w / w.sum().clamp_min(1e-6)
            loss = loss + (w * (idx[lo:hi] - float(ti)).abs() / norm).sum()
            n_main += 1

        # explizite miss / fp kosten
        if miss:
            loss = loss + miss_cost * len(miss)
            n_main += len(miss)
        if fp:
            loss = loss + fp_cost * e[b, fp].sum()
            n_main += len(fp)

        # nur für unbediente targets lokaler birth-term
        for ti in miss:
    ...
    L_dup = (u - 1.0).relu().pow(2).mean()

    return loss / max(n_main, 1) + lambda_birth * birth / max(n_birth, 1) + lambda_dup * L_dup

# =========================
# train / eval
# =========================
@torch.no_grad()
def evaluate(model, loader, device, move_scale=2.0, miss_cost=1.0, fp_cost=1.0, lambda_birth=0.4, birth_r=1, lambda_dup=0.05):
    model.eval()
    total, n, spk, tgt = 0.0, 0, 0.0, 0.0
    for xb, yb in loader:
        xb, yb = xb.to(device), yb.to(device)
        out = model(xb)
        e = (out["s"].sum(dim=1) > 0).float()              # Event-Spikes für Metrik/Regler
        total += event_loss(out["z"], out["s"], yb, move_scale, miss_cost, fp_cost, lambda_birth, birth_r, lambda_dup).item() * xb.size(0)
        n += xb.size(0)
        spk += e.sum().item()
        tgt += yb.sum().item()
...
def fit(model, train_loader, test_loader, epochs=20, lr=1e-3, device="cpu",
        move_scale=2.0, miss_cost=1.0, fp_cost=1.0, lambda_birth=0.4, birth_r=1, lambda_dup=0.05):
    ...
    for ep in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            xb, yb = xb.to(device), yb.to(device)
            out = model(xb)
            e = (out["s"].sum(dim=1) > 0).float()          # Event-Spikes für Regler
            loss = event_loss(out["z"], out["s"], yb, move_scale, miss_cost, fp_cost, lambda_birth, birth_r, lambda_dup)
        ...
        tr_loss = total / n
        te_loss, te_spk, te_tgt = evaluate(model, test_loader, device, move_scale, miss_cost, fp_cost, lambda_birth, birth_r, lambda_dup)
        ...
        print(
            ...
            f"gain {model.gain.item():.3f} | train event-spikes {int(train_spk)} | train targets {int(train_tgt)} | "
...
def plot_sequence(model, loader, device="cpu", idx=0):
    ...
    plt.title(f"idx={idx} | gain={model.gain.item():.3f} | targets={int(y.sum())} | event spikes={int(e.sum())}")
    plt.tight_layout()
    plt.show()
...
model, hist = fit(
    ...
    lambda_birth=0.4, birth_r=1, lambda_dup=0.05
)
