...
class SNNFilterBank(nn.Module):
    ...
    def forward(self, x_window):
        ...
        for k in range(self.n_filters):
            ...
            gated_input = proj * (1.0 - inh_spike)
...
class SNN101(nn.Module):
    ...
    def reset(self, batch_size, device):
        self.bank.reset(batch_size, device=device)
...
if __name__ == "__main__":
    ...
    s1 = torch.sin(2.0 * t)  # class 1 (different frequency)
    ...
    for x_windows, y in dataloader:
        x_windows = x_windows.permute(1, 0, 2)  # [batch,time,window] -> [time,batch,window]
