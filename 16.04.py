...
class SNNEventModel(nn.Module):
    def __init__(
        ...
        self.kp = float(kp)
        self.register_buffer("gain", torch.ones(2))

    def forward(self, x):                                # x: [B,2,L]
        Ia = self.gain[0] * self.bank_a(x[:, 0:1])
        Ib = self.gain[1] * self.bank_b(x[:, 1:2])
    ...
    def p_update_epoch(self, spike_sum, target_sum, spike_sites):
        target_per_branch = target_sum / 2
        err = (target_per_branch - spike_sum) / max(target_per_branch, 1.0)
        self.gain.mul_(torch.exp(self.kp * err))
        return err
...
def event_loss(z, s, y):
    ...
    for zb, sb, yb in zip(z, s, y):
        targets = yb.nonzero(as_tuple=False).flatten().tolist()
        spikes = [tuple(map(int, x)) for x in sb.nonzero(as_tuple=False).tolist()]
        disc, sur = 0.0, z.new_zeros(())
        for c, ts in spikes:
            if targets:
                ty = min(targets, key=lambda t: abs(t - ts))
                d = abs(ty - ts)
                w = d / L
                disc += d
                sur = sur - w * F.logsigmoid(zb[c, ty])
                if ts != ty:
                    sur = sur - w * F.logsigmoid(-zb[c, ts])
            else:
                disc += L
                sur = sur - F.logsigmoid(-zb[c, ts])
        disc = z.new_tensor(disc)
...
@torch.no_grad()
def calibrate_gain(model, loader, epochs=100, device="cpu"):
    model.to(device)
    for ep in range(1, epochs + 1):
        model.eval()
        spk = torch.zeros(2, device=device)
        tgt, sites = 0.0, torch.zeros(2, device=device)
        for xb, yb in loader:
            xb, yb = xb.to(device), yb.to(device)
            out = model(xb)
            h = out["s"].size(1) // 2
            spk += torch.stack([out["s"][:, :h].sum(), out["s"][:, h:].sum()])
            tgt += yb.sum().item()
            sites += torch.tensor([out["s"][:, :h].numel(), out["s"][:, h:].numel()], device=device)
        err = model.p_update_epoch(spk, tgt, sites)
        print(
            f"Gain {ep:03d} | gain {model.gain[0].item():.3f}/{model.gain[1].item():.3f} | "
            f"spikes {int(spk.sum().item())} ({int(spk[0].item())}/{int(spk[1].item())}) | "
            f"targets {int(tgt)} | err {err[0].item():.5f}/{err[1].item():.5f}"
        )

def fit(model, train_loader, test_loader, epochs=20, lr=1e-3, device="cpu", gain_every=5, gain_epochs=3):
    ...
    for ep in range(1, epochs + 1):
        model.train()
        total, n = 0.0, 0
        train_spk = torch.zeros(2, device=device)
        train_tgt = 0.0
        ...
        for xb, yb in train_loader:
            ...
            h = out["s"].size(1) // 2
            train_spk += torch.stack([out["s"][:, :h].sum(), out["s"][:, h:].sum()]).detach()
        ...
        hist["gain"].append(model.gain.detach().cpu().tolist())

        print(
            f"Epoch {ep:03d} | train {tr_loss:.4f} | test {te_loss:.4f} | "
            f"gain {model.gain[0].item():.3f}/{model.gain[1].item():.3f} | "
            f"train spikes {int(train_spk.sum().item())} ({int(train_spk[0].item())}/{int(train_spk[1].item())}) | "
            f"train targets {int(train_tgt)} | "
            f"test spikes {int(te_spk)} | test targets {int(te_tgt)}"
        )
        if gain_every and ep % gain_every == 0:
            calibrate_gain(model, train_loader, epochs=gain_epochs, device=device)
...
def plot_sequence(model, loader, device="cpu", idx=0):
    ...
    s = out["s"][0].cpu()
    h = s.size(0) // 2
    cf_spk, ed_spk = int(s[:h].sum()), int(s[h:].sum())
    ...
    plt.title(
        f"idx={idx} | gain={model.gain[0].item():.3f}/{model.gain[1].item():.3f} | "
        f"targets={int(y.sum())} | spikes={int(s.sum())} ({cf_spk}/{ed_spk})"
    )
    plt.tight_layout()
    plt.show()

    print(f"idx {idx:03d} | targets {int(y.sum().item())} | spikes {int(s.sum().item())} ({cf_spk}/{ed_spk})")
...
@torch.no_grad()
def top_branch_indices(model, loader, device="cpu", branch=0, n=4):
    model.eval()
    scores = []
    for i in range(len(loader.dataset)):
        x, _ = dataset_sample(loader, i, device)
        s = model(x)["s"][0].cpu()
        h = s.size(0) // 2
        part = s[:h] if branch == 0 else s[h:]
        scores.append((int(part.sum()), i))
    return [i for _, i in sorted(scores, reverse=True)[:n]]
...
calibrate_gain(model, train_loader, epochs=10, device=device)
model, hist = fit(
    model, train_loader, test_loader,
    epochs=100, lr=1e-3, device=device, gain_every=5, gain_epochs=3,
)
...
plot_sequences(model, test_loader, device=device, idxs=top_branch_indices(model, test_loader, device=device, branch=0, n=4))
