...
class DendriticMatcherBank(nn.Module):
    def __init__(self, T, K, tau_psp=40.0, tau_mem=20.0, v_th=1.0, dt=1.0):
        super().__init__()
        self.T = T
        self.K = K
        ...
        self.v_th = v_th          # Schwelle pro Matcher
    ...
    def forward(self, spike_times, return_spike_times: bool = False):
        ...
        # -----------------------------
        # Expand input spike_times auf K Matcher
        ...
        # Referenzzeit: max arrival pro Matcher (wie zuvor)
        ...
        # -----------------------------
        # 3) Matcher-"Potenzial" wie früher (für Kompatibilität)
        #    = Summe über Zeit
        # -----------------------------
        pot = torch.sum(I, dim=2)                                # (B, K)

        # -----------------------------
        # 4) LIF pro Matcher -> Spikezeit (optional)
        # -----------------------------
        if not return_spike_times:
            # Altes Interface: keine Spikezeiten benötigt
            return pot, arrivals, psp

        # LIF-Integration über die T-Schritte: v_{t+1} = v_t + (-v_t + I_t) * dt/tau_mem
        v = torch.zeros(B, self.K, device=device)
        spike_idx = torch.full((B, self.K), fill_value=T, device=device, dtype=torch.long)
        spiked = torch.zeros(B, self.K, device=device, dtype=torch.bool)
        ...
        for t in range(T):
            ...
            crossed = (v >= self.v_th) & (~spiked)
            # erster Zeitpunkt, an dem der jeweilige Matcher feuert
            spike_idx[crossed] = t
            spiked = spiked | crossed

        # Diskrete Spikezeit als "t * dt"
        matcher_spike_times = spike_idx.to(torch.float32) * self.dt  # (B, K)
...
class ThreeLevelCombiner(nn.Module):
    ...
    def _psp_exp(self, t, spike_times, tau_syn):
        """
        Exponentieller PSP-Kern:
            PSP(t) = exp(-(t - ts)/tau) für t >= ts, sonst 0
    ...
    def forward(self, matcher_spike_times):
        ...
        for step in range(self.n_steps_L1):
            ...
            # PSPs der beiden Eingänge: (B, n_L1, 2)
            psp = self._psp_exp(t, L1_inputs, self.tau_syn_L1)

            # gewichteter Strom: (B, n_L1)
            I_L1 = torch.sum(
        ...
        idx_L1 = torch.argmax(v_L1_trace, dim=-1)      # (B, n_L1)
        L1_spike_times = idx_L1.to(torch.float32) * self.dt  # (B, n_L1)
        ...
        idx_L2 = torch.argmax(v_L2_trace, dim=-1)         # (B, n_L2)
        L2_spike_times = idx_L2.to(torch.float32) * self.dt  # (B, n_L2)

        # ================================
        # Soma: n_L2 -> 1 Spikezeit (mit Schwelle)
        # ================================
        v_soma = torch.zeros(B, device=device)
        spiked = torch.zeros(B, device=device, dtype=torch.bool)
        spike_idx_soma = torch.full(
            (B,), fill_value=self.n_steps_soma, device=device, dtype=torch.long
        )
        ...
        for step in range(self.n_steps_soma):
            ...
            psp = self._psp_exp(t, soma_inputs, self.tau_syn_soma)  # (B, n_L2)
            ...
            crossed = (v_soma >= self.soma_threshold) & (~spiked)
            spike_idx_soma[crossed] = step
            spiked = spiked | crossed

        soma_spike_times = spike_idx_soma.to(torch.float32) * self.dt  # (B,)
...
class FullModel(nn.Module):
    ...
    def forward(self, latencies):  # latencies: (B, channels, T)
        ...
        for c in range(self.channels):
            # Matcher gibt jetzt Spikezeiten der K-Matcher zurück
        ...
        #    die Soma-Spikezeit des Zielkanals rekonstruieren
        C = self.channels
        v = torch.zeros(B, C, device=device)
        spiked = torch.zeros(B, C, device=device, dtype=torch.bool)
        spike_idx = torch.full(
            (B, C), fill_value=self.cross_n_steps,
        ...
        for step in range(self.cross_n_steps):
            ...
            crossed = (v >= self.cross_threshold) & (~spiked)
            spike_idx[crossed] = step
            spiked = spiked | crossed

        recon_soma_times = spike_idx.to(torch.float32) * self.cross_dt  # (B, C)
...
for epoch in range(1, num_epochs+1):
    ...
    for (batch_lat,) in train_loader:
        ...
        if grad is not None:
            ...
            print("delay grad norm: None (kein Gradfluss durch das aktuelle Spiking-Setup)")
